...
class TicTacToe:
    ...
    def drawBoard(self, board=None):
        if board is None:
            print(self.form %
                  tuple(self.board[7:10] + self.board[4:7] + self.board[1:4]))
        else:
    ...
    def getPlayerMove(self):
        ...
        while move not in range(1,
                                10) or not self.isSpaceFree(self.board, move):
    ...
    # 游戏结束
    def endGame(self):
        playAgain = input("希望再战一局吗? (y/n): ").lower()
        if playAgain == 'y':
            self.__init__()
            self.startGame()
        else:
