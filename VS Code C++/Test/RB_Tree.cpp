/*
    红黑树的性质：
        ...
        5.
   每个红色节点的两个子节点都是黑色（即从每个叶子节点到根节点的所有路径上不能有两个连续的红色节点）
        6.
   从根节点到叶子节点的所有路径都包含相同数目的黑色节点（黑高度）（若节点只有一个儿子则肯定是黑结点）
        7. 红黑树可变为等价B树
    节点：
    ...
    插入与双红现象：
        为了尽可能维护性质6，每一次插入都要将节点作为红色节点插入。BST的插入应该比较容易实现，但问题在于，性质
   5可能被破坏，
    ...
    双红修正：
        ...
        RR-1（叔叔是黑色）（插入到等价 B-树的三节点中）：
            如果正在修正的节点的父亲是红色（祖父一定是黑色），但是叔叔是黑色，那么只需要做
   1或 2次旋转，再进行 2次染色就可
            以解决（同时也是RR-2的递归基）。
    ...
    失黑修正：
        失黑修正有四种情况（根据父亲和兄弟颜色划分），不容易看出，做到心中有
   B-树
        LB-1B（父亲为黑色，兄弟为红色）：
...
template <typename T> class redblacktree {
  ...
  public:
    ...
    redblacktree() : _root(NULL), f_root(NULL), _size(0) {}
...
template <typename T> struct redblacktree<T>::RB_Node {
    T val;
    RB_COLOR RBc;
    RB_Node *ftr;
    RB_Node *lc;
    ...
    RB_Node *left_node() {
        RB_Node *ptn = this;
        if (!lc) { //如果当前节点（无左儿子）仍是父节点的左儿子，向上查找，直到节点是右儿子为止，则该节点的父节点就是要找的前驱节点
            while (ptn->ftr && ptn->ftr->lc == ptn) {
                ptn = ptn->ftr;
            }
            ptn = ptn->ftr;
        } else { //若有左儿子，则一直向左儿子的右儿子寻找
            ptn = ptn->lc;
            while (ptn->rc) {
                ptn = ptn->rc;
            }
...
template <typename T> struct redblacktree<T>::iterator { //开始重载运算符
  protected:
    RB_Node *_real__node;
...
//开始插入
template <typename T> void redblacktree<T>::init(T v) { //初始化，根节点为黑色
...
typename redblacktree<T>::iterator redblacktree<T>::insert(T v) {
    ...
    if (ptn)
        return iterator(ptn);
    if (!f_root) // assert(_size == 0)
    {
        init(v);
        return iterator(_root);
    }
    ++_size;
    ptn = new RB_Node(v, RB_COLOR_RED, f_root, NULL, NULL);
    if (f_root->val < v)
        f_root->rc = ptn;
    else
        f_root->lc = ptn;
...
//双红修正
template <typename T> void redblacktree<T>::SolveDoubleRed(RB_Node *nn) {
    while (nn->ftr &&
           nn->ftr->RBc == RB_COLOR_RED) { //排除递归到根和 RR-0的情况
        RB_Node *pftr = nn->ftr;
...
template <typename T> int redblacktree<T>::size() { return _size; }

template <typename T> bool redblacktree<T>::empty() { return !_size; }
...
//内部接口
template <typename T> void redblacktree<T>::removetree(RB_Node *ptn) {
    if (!ptn)
        return;
    removetree(ptn->lc);
    removetree(ptn->rc);
...
template <typename T> void redblacktree<T>::SolveLostBlack(RB_Node *nn) {
    while (nn != _root) {
        RB_Node *pftr = nn->ftr;
        RB_Node *bthr = bro(nn);
...
//删除
template <typename T> bool redblacktree<T>::remove(T v) {
    RB_Node *ptn = find(v);
...
template <typename T> bool redblacktree<T>::remove(iterator &itr) {
    RB_Node *ptn = itr._real__node;
    itr._real__node =
        itr._real__node
            ->right_node(); //进行维护，保证删除过后iterator仍旧指向正常结点
    if (!(itr._real__node))
